...
export function OrganismSelector({ value, onChange }: Props) {
  return (
    <div>
      <label className="block text-xs font-medium text-stone-500 dark:text-stone-400 mb-1 uppercase tracking-wider">Target Organism</label>
      <select
        value={value}
        onChange={e => onChange(e.target.value as OrganismId)}
        className="w-full px-2 py-1.5 text-xs bg-white dark:bg-stone-800 border border-stone-300 dark:border-stone-600 rounded focus:outline-none focus:border-stone-500"
      >
