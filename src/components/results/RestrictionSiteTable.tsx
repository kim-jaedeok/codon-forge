...
export function RestrictionSiteTable({ sites }: { sites: RestrictionSite[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {sites.length === 0 ? (
        <p className="text-sm text-gray-500 p-4 text-center">No restriction enzyme cut sites found.</p>
      ) : (
        <div className="space-y-2">
          {enzymes.map(enzyme => (
            <div key={enzyme} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              <div className="flex items-center justify-between mb-1">
                <span className="font-semibold text-sm">{enzyme}</span>
                <span className="text-xs text-gray-500 font-mono">{grouped[enzyme][0].recognitionSequence}</span>
              </div>
