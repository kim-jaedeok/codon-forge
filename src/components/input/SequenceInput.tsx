...
export function SequenceInput({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
      <h2 className="text-xl font-semibold mb-3 text-gray-900 dark:text-white">Sequence Input</h2>
      ...
      {error && (
        <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-base text-red-700 dark:text-red-400">
          {error}
        </div>
      )}

      {warnings.length > 0 && (
        <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg text-base text-yellow-700 dark:text-yellow-400">
          {warnings.map((w, i) => <p key={i}>{w}</p>)}
        </div>
      )}

      {sequenceInfo && (
        <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg text-base">
