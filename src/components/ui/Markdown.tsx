...
function parseLine(line: string): ReactNode {
  ...
  processed = processed.replace(/`([^`]+)`/g, (_m, code, offset) => {
    ...
    codeSegments.push(
      <code key={`c${offset}`} className="px-1 py-0.5 bg-stone-100 dark:bg-stone-700 rounded text-[11px] font-mono">
        {code}
      </code>
    );
    return placeholder;
...
export function Markdown({ text }: Props) {
  ...
  while (i < lines.length) {
      ...
      elements.push(
        <div key={`code${i}`} className="my-2 rounded border border-stone-200 dark:border-stone-600 overflow-hidden">
          ...
          <pre className="p-2 bg-stone-50 dark:bg-stone-800 overflow-x-auto">
            <code className="text-[11px] font-mono text-stone-700 dark:text-stone-300 leading-relaxed">
              {codeLines.join('\n')}
            </code>
          </pre>
        </div>
    ...
    if (line.startsWith('## ')) {
      elements.push(
        <h3 key={i} className="text-xs font-semibold text-stone-800 dark:text-stone-200 uppercase tracking-wider mt-4 mb-1.5 pb-1 border-b border-stone-200 dark:border-stone-700">
          {line.slice(3)}
        </h3>
      );
      i++;
    ...
    if (line.startsWith('### ')) {
      elements.push(
        <h4 key={i} className="text-xs font-semibold text-stone-700 dark:text-stone-300 mt-3 mb-1">
          {line.slice(4)}
        </h4>
      );
      i++;
      ...
      elements.push(
        <ul key={`ul${i}`} className="space-y-0.5 my-1">
          {items.map((item, j) => {
            ...
            return (
              <li key={j} className="text-xs text-stone-600 dark:text-stone-400 flex gap-1.5" style={{ paddingLeft: depth * 12 }}>
    ...
    if (/^\d+\. /.test(line)) {
      ...
      elements.push(
        <ol key={`ol${i}`} className="space-y-0.5 my-1">
          {items.map((item, j) => (
            <li key={j} className="text-xs text-stone-600 dark:text-stone-400 flex gap-1.5">
              <span className="text-stone-400 font-mono text-[10px] mt-0.5 shrink-0 w-3 text-right">{j + 1}.</span>
    ...
    // Regular paragraph
    elements.push(
      <p key={i} className="text-xs text-stone-600 dark:text-stone-400 leading-relaxed my-1">
        {parseLine(line)}
      </p>
    );
    i++;
